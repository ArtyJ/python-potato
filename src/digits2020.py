...
def findGHJK():
    ...
    for number in range(1023, 9877):
        if len(set(list(map(int, str(number))))) < 4:
            continue
        gh = number // 100
        jk = number % 100
        if jk > 9 and gh % jk == 0:
...
def findDigits(listGHJK):
    ...
    for abcdef in range(102345, 987655):
        ...
        if len(d6) != len(set(d6)):
            continue  # duplicate digit in abcdef
        for ghjk in listGHJK:
            # convert number to a list of digits
            d4 = list(map(int, str(ghjk)))
            if len(set(d4) & set(d6)) > 0:
                continue  # abcdef duplicates digit with ghjk
            combination += 1
            ab = abcdef // 10000
            cd = abcdef // 100 % 100
            ef = abcdef % 100
            ...
            if ab*cd+ef-gh/jk == target:
                solution += 1
                print("No.%d: %02dx%02d+%02d-%02d/%02d=%d" %
                      (solution, ab, cd, ef, gh, jk, target))
